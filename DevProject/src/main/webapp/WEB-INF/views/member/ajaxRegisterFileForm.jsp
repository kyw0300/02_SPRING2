...
<head>
<meta charset="UTF-8">
<title>Ajax Register File Form</title>
...
$(function(){
	$("#inputFile").on("change", function(event){
		...
		if(isImageFile(file)) { // 이미지 파일 일 때
			// 비동기 처리 시, 파일데이터를 전송할 때에는 formData()를 이용하여 데이터를 전송한다.
			var formData = new FormData();
			formData.append("file", file);
			
			// formData는 key/value 형식으로 데이터가 저장된다.
			// dataType : 응답(response) 데이터를 내보낼 때 보내줄 데이터 타입
			// processData : 데이터 파라미터를 data라는 속성으로 넣는데, jquery 내부적으로 쿼리스트링을 구성합니다.
			...
			//					request 요청에서 Content-Type을 확인해보면 'multipart/form-data; boundary=----WebKitFormBoundaryxFXsF60mYAq3MyWy'
...
function isImageFile(file){
	...
	// 확장자 중 이미지 확장자가 포함되어 있아면 0보다 큰 수 일테니 true가 리턴
