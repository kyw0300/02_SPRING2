...
public class DatabaseConnectController {
	 ...
	 * 		3. 게시판 구현
	 * 
	 * 			** 기본적인 JDBC를 이용한 DB설계는 DAO란에서 sql 쿼리가 노출되어 개발하는 방식이다보니 우리가 중급프로젝트때 또는 JSP에서
	 * 			preparestatement방식의 데이터베이스 연동과 다를바 없는 형태로, 게시판 구현을 진행하지 않고 내영만 간략히 보고 넘어가도록 한다.
	 * 
	 ...
	 *      - EntityManager에서 제공하는 Entity 조직 API를 이용해 Entity에 대해 CRUD 작업을 할 수 있다.
	 *      
	 *      # Entity 상태
	 *      - new 상태
	 *      - 관리 상태
	 ...
	 *      
	 *      # JPA 관련 어노테이션
	 *      자바빈즈 클래스 객체가 곧 테이블 구조와 같은 형태이기 때문에, VO클래스 내 @Entity 어노테이션을 통해 해당 객체가 테이블과 같은 Entity임을
	 *      설정하고 @TaBle 어노테이션으로 테이블명을 설정해주면 VO클래스는 데이터베이스의 생성되어 있는 테이블과 연결되기 위한 준비가 된다.
	 *      
	 *      ex)
	 *      	@Entity
	 *      	@Table(name="board")
	 ...
	 *      설정된 Entity들을 이용하여 jpa객체를 통해서 함수를 호출하면(persist, find 등등) 호출된 함수를 통해 Spring Data JPA가
	 ...
	 *      그리고, 사용하는 데이터베이스 종류가 변경된다 하더라도 문제가 없다.(JPA는 SQL을 직접 건드리지않고 함수와 같은 기능들로만 쿼리를 요청하고
	 *      데이터베이스와 통신하기 때문입니다!)
	 *      
	 *      JPA는 JPA구현체인 hibernate을 이용하여 사용합니다. 그러기 위해선 관련 의존관계를 등록 후 사용해야합니다.
	 ...
	 * 		- 저는 ibatis를 사용했습니다. 저는 mybitis를 사용했습니다. 와 같은 대답을 하시면 됩니다!
	 * ---------------------------------------------------------------------------
	 * 
	 * 7. 마이바티스
	 ...
	 * 		- 예제로 mapper 패키지 안에 BoardMapper.xml을 구성하여 테스트를 진행합니다!
	 */
}
