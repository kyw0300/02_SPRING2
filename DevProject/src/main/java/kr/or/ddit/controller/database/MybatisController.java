...
public class MybatisController {
	 ...
	 * 
	 * 4. '_'로 구분된 컬럼명 자동 매핑
	 * - 마이바티스 설정의 mapUnderscoreToCamelCase 프로퍼티 값은 true로 지정하면 '_'로 구분된 컬럼명을 소문자 낙타표기법의
	 * 		프로퍼티명으로 자동 매핑할 수 있다.
	 * 
	 ...
	 * 				- </settings>
	 * 
	 * 			1-2) 매핑 파일 수정(기본 Mapper xml에 설정된 컬럼은 board_no와 같은 형태로 되어 있다고 가정)
	 ...
	 * 				최종 컨트롤러까지 넘어간다.
	 * 
	 * 				*** seq_board.nextval대신 현재의 seq번호를 가져오기 위래 currval를 사용하게 되는 경우가 있다.
	 * 					이때 사용시 주의사항이 있다.
	 * 
	 ...
	 * 		- 우리는 그러한 공통부 들을 분리하여 반복적으로 컴포넌트들을 사용하는게 아니라 공통적인 부분은 한번만 가져다 쓰고
	 ...
	 * 
	 * 			그 외에 다양한 영역의 페이지는 구현하고자 하는 시나리오를 바탕으로 페이지가 구성될 때 추가적으로 레이아웃 영역을 분리하며
	 * 			작성하면 됩니다.
	 * 
	 * 2. Tiles 설정
	 * 
