...
public class TransactionController {
	 ...
	 * 			클래스나 메서드에 대해 @Transactional 어노테이션을 부여하여 트랜잭션을 적용할 수 있습니다.
	 * 
	 * 		서비스 구현 객체에 트랜잭션 적용
	 ...
	 * 		기본적으로 스프링에선 트랜잭션 처리는 RuntimeException 계열이라면 Rollback 처리를 한다.
	 * 		그러나 UncheckedException에 대해서는 Rollback 처리를 진행하지 않는다.
	 * 
	 * 		여기서, 트랜잭션으로 국한된 롤백 정책이 아니라 스프링 프레임워크에서의 기본 정책에 대한 내용일 뿐이다.
	 * 		실제로 '트랜잭션 롤백 처리', 'checked Exception vs unchecked Exception' 등 검색해보면 잘못된 정보들로 복사/붙여넣기 되어 있는 내용이
	 * 		수두룩하여 많은 에러 정보를 공유하고 있다. (모든 블로그가 그렇진 않지만, 대표적인 표가 무수히 많이 공유되고 있다.)
	 * 
	 * 		***
	 * 		'예외처리 시 트랜잭션 처리'에 대한 정보가 잘못되어 표기된 경우가 많다.
	 * 		이럴 때 되고 저럴 때 되고가 아니라, 기본적인 스프링 트랜잭션의 정책이 있고 그 안에서 정책 이외에 옵션으로 특정 에러가 발생했을 때
	 * 		롤백을 진행할 수 있도록 제공한다. (많은 블로그들을 100% 믿지 말길 바랍니다... 혹여나 블로그를 작성한다면 검증된 내용으로만 적어주길 바랄게요...)
	 * 
	 ...
	 *  	─────────────────────────────────────────────────────────────────────────────────
	 *  
	 *  	***** 트랜잭션도 AOP의 개념이 반영된 관점 지향 프로그래밍이다 할 수 있습니다.
	 *  
	 ...
	 *  
	 *  		1) isolation (격리수준)
	 *  			- 각 트랜잭션이 존재할 때, 트랜잭션들끼리 서로 고립된 수준을 나타내며 서로간에 가용된 데이터를 컨트롤 할지에 대한 부분들을 설정할 수 있다.
	 ...
	 *  			- 추가적인 트랜잭션을 기존 트랜잭션에 포함시켜 함께 처리할수도 있고, 추가적인 트랜잭션처럼 별도의 트랜잭션으로 추가할 수도 있고
	 ...
	 *  					> Existing transaction found for transaction marked with propagetion 'never' 에러가 발생한다.
	 *  
	 *  		3) readOnly(읽기 전용 설정)
	 ...
	 *  		4) timeout (트랜잭션 제한시간)
	 *  			- 기본값은 -1로 무제한입니다.
	 *  			- timeout은 클라이언트와 서버와의 통신 중, 서버 측 문제로 다음 처리를 이어나가지 못하는 'Deadlock'을 방지할 수 있는 속성입니다.
