...
public class ValidationController {
	 ...
	 * 
	 * 1. 입력값 검증
	 * - 스프링 MVC Bean Validation 기능을 이용해 요청 파라미터 값이 바인딩괸 도메인 클래스(또는 커맨드 클래스)의 입력값 검증을 한다.
	 * 
	 * [환경설정]
	 * # 의존 관계 정의
	 ...
	 * 
	 * # 입력값 검증 활성화
	 * - Member 클래스로 넘어가서 임시 테스트로 userId, userName에 에 규칙을 활성화한다.
	...
	@PostMapping("/result2")
	public String registerValidatedFormResult2(@Validated Member member, BindingResult result) {
		log.info("registerValidatedFormResult2() 실행...!");
	...
	/*
	 * 3. 입력값 검증 규칙
	 * - 입력값 검증 규칙은 Bean Validation이 제공하는 제약 어노테이션으로 셜정한다.
	 * 
	 ...
	 * 		- 직접 구현한 제약 어노테이션
	 * 
	 * 		1) Member 클래스에서 테스트를 위한 어노테이션으로 설정(아래 명시되어 있는 내용들을 가지고 테스트 해보세요들!!!)
	 ...
	 *		- 테스트 URL : /validation/registerValidationForm02로 요청해서 테스트를 진행해봅시다!
	 *
	 *
	 * 4. 중첩된 자바빈즈 입력값 검증
	...
	@PostMapping("/result3")
	public String registerValidatedFormResult3(@Validated Member member, BindingResult result) {
		log.info("registerValidatedFormResult3() 실행...!");
