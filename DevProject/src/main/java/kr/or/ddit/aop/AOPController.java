...
public class AOPController {
	 ...
	 * 		그러던 어느 날, 성희 팀장님을 통해서 '반장님, 지금 개발중인 서비스 처리 속도 좀 로그로 남겨주세요!'라는 부탁을 받습니다.
	 ...
	 * 		정수학생은 본인이 만들어 낸 메소드를 각 기능별 서비스 로직에 하나하나씩 약 2만개를 넣을쯤에 의문을 갖습니다...
	 * 
	 * 			'서비스 로직에서 제일 중요한 로직은 본래의 기능이 제일 중요하고 지금 내가 작성하는 로직은 옵션(부가 기능)이 추가되는게 아닐려나?'
	 ...
	 * 
	 * 		이런 횡단 관심사를 통해서 프로그래밍하는 것이 AOP라고 한다.
	 * 
	 * 			** 간단하게 맛보기
	 * 			- Aspect(어스펙트) : AOP의 단위가 되는 횡단 관심사
	 * 			- 횡단 관심사(Croess-Cutting Concern) : 핵심(Core) 비즈니스 로직(메소드 실행 시작 시간 출력, 메소드 처리 후 시간 출력 등)과 다소 거리가 있지만,
	 ...
	 * 			- Advice : 어떤 부가기능(메소드 실행 시작 전, 메소드 실행 후 등)을 언제(삼겹살을 굽기 전, 삼겹살을 다 먹을 후 등) 사용할지 정의
	 * 				* 언제?
	 ...
	 * 				> Around : 조인 포인트 전후에 실행(삼겹살을 굽기 전과 먹을 후 실행)
	 * 
	 * 
	 ...
	 * 
	 * 		- 횡단 관심사
	 * 		> 핵심 비즈니스 로직과 다소 거리가 있지만, 여러 묘듈에서 공통적이고 반복적인 처리를 요구하는 내용이다.
	 * 
	 * 		- 횡단 관심사의 분리
	 ...
	 * 		- 예외 처리
	 * 
	 * 			** AOP의 사용된 예에서 우리는 로깅을 이용해 볼 것입니다.
	 * 				기본적인 보안적용은 스프링 시큐리티에서 사용하고, 트랜잭션 관리는 @Transantional 어노테이션을 활용한 트랜잭션에서 사용하고,
	 ...
	 * 
	 * 		[AOP 프록시]
	 * 		클라이언트가 요청한 해당 요청을 서비스 로직으로 전달되지 전 해당 비즈니스 로직 부를 실제 대상인것처럼 위장해서 클라이언트와 요청을 받는다.
