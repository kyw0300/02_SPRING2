 ...
 *  	로그인 페이지 정의
 *  	- 사용자가 정의한 로그인 컨트롤러
 *  		> controller패키지 안에 LoginController 메소드 생성
 *  	- 사용자가 정의한 로그인 뷰
 *  		> views/loginForm.jsp
 *  
 *  		*** 시큐리티에서 제공하는 기본 로그인 페이지로 이동하지 않고,
 *  			사용자가 정의한 로그인 페이지의 URI를 요청하여 해당 페이지에서 권한을 체크하도록 합니다.
 *  			인증이 완료되면 최초의 요청된 target URI로 이동합니다. 그렇지 않은 경우 사용자가 만들어놓은
 *  			접근 거부 페이지로 이동합니다.
 *  
 * 8. 로그인 성공 처리
 *  - 로그인을 성공한 후에 로그인 이력 로그를 기록하는 등의 동작을 하고 싶은 경우가 있습니다.
 *  	이런 경우에 AuthenticationSuccessHandler이라는 인터페이스를 구현해서 로그인 성공 처리자로 지정할 수 있습니다.
 *  
 *  	환경설정
 *  	- 스프링 시큐리티 설정
 *  		> security-context.xml 설정
 ...
 *  			인증 전에 접근을 시도한 URL로 리다이렉트하는 기능을 가지고 있으며 스프링 시큐리티에서 기본적으로 사용되는 구현 클래스입니다.
 *  
 *  	화면 설명
 *  	- 일반 게시판 등록 화면
 *  		> 사용자가 정의한 로그인 페이지에서 회원 권한에 해당하는 계정으로 로그인 시, 성공했다면 성공 처리자인 CustomLoginSuccess 클래스로 넘어가
 *  			넘겨받은 파라미터들 중 authentication안에 principal로 User 정보를 받아서 username과 password를 출력한다.
 ...
 *  	비밀번호 암호화 처리가 클래스 정의
 *  	- 비밀번호 암호화 처리기
 *  	> 스프링 시큐리티 5버전부터는 기본적으로 PasswordEncoder를 지정해야 하는데,
 *  		제대로 하려면 생성된 사용자 테이블(users)에 비밀번호를 암호화하여 저장해야 합니다.
 *  		테스트를 위해서 생성한 데이터는 암호화를 처리하지 않고 로그인하면 당연히 로그인 에러가 발생 할 것입니다.(암호화된 비밀번호가 날라가는게 아니라서)
 *  		그래서 암호화를 하지 않는 PasswordEncoder를 직접 구현하여 지정하면 로그인 시 암호화를 고려하지 않으므로 로그인이 정상적으로 이루어지는걸 확인할 수 있습니다.
 *  		- security/CustomNoOpPasswordEncoder 클래스 구현
 * 11. 사용자 테이블 이용한 인증/인가 처리
 * 스프링 시큐리티가 기본적으로 이용하는 테이블 구조를 그래로 생성해서 사용해도 되지만,
 ...
 *  	> 12번째 해당하는 내용에서 자세한 내용으로 확인
 *  
 *  		BCryptPasswordEncoder 클래스를 활용한 단방향 비밀번호 암호화
 *  		- encode() 메서드를 통해서 SHA-2 방식의 8바이트 Hash 암호를 매번 랜덤하게 생성합니다.
 *  		- 똑같은 비밀번호를 입력하더라도 암호화되는 문자열은 매번 다른 문자열을 반환한다.
 *  
 *  	> 비밀번호를 입력하면 암호화된 비밀번호로 인코딩되는데, 암호화된 비밀번호와 DB테이블에 있는 암호화된 비밀번호가 일치한지를 파악 한 후,
 *  		일치하면 로그인 성공으로 다음 스텝을 진행합니다.
 *  	- BCryptPasswordEncoder 클래스의 encode() 메소드를 통해 만들어지는 암호화된 해쉬 다이제스트들은 입력한 비밀번호 문자에 해당하는
 *  		수십억개의 다이제스트들 중에서 일치하는 다이제스트가 존재할 경우 비밀번호의 일치를 보고 인증을 성공시켜줍니다.
 * 
 ...
 * 		> <security:remember-me data-source-ref="dataSource" token-validity-second="604800" /> 태그 설정
 * 		> <security:logout logout-url="/logout" invalidate-session="true" delete-cookies="JSESSION_ID, remember-me" /> 태그 설정
 * 
 * 		자동 로그인
 * 		- 로그인 상태 유지 체크박스 추가
 * 		> loginForm.jsp 수정
 ...
 * 		- JSESSION와 remember-me 쿠키가 만들어진다.
 * 		- JSESSIONID를 삭제 후, 다시 로그인을 진행하더라도 로그인 후 진행될 페이지가 정상적으로 나타나는걸 확인할 수 있습니다.
 * 			> 자동 로그인이 remember-me에 의해 실행됨을 확인할 수 있습니다.
 * 
 * 15. 스프링 시큐리티 어노테이션
